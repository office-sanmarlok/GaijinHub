-- Add romanized fields to get_listing_details RPC
...
BEGIN
  ...
  SELECT 
    ...
    -- Station info with romanized fields
    CASE 
      WHEN l.station_g_cd IS NOT NULL THEN
        (SELECT jsonb_build_object(
          'station_g_cd', sg.station_g_cd,
          'station_name', sg.station_name,
          'station_name_r', sg.station_name_r,
          'station_name_h', sg.station_name_h,
          'lat', sg.lat,
          'lng', sg.lng,
          'lines', COALESCE(
            ...
            FROM stations s
            JOIN lines ln ON s.line_cd = ln.line_cd
            JOIN companies c ON ln.company_cd = c.company_cd
        ...
        WHERE sg.station_g_cd = l.station_g_cd)
      ELSE NULL
    END as station,
    -- Municipality info with romanized fields
    CASE 
      WHEN l.muni_id IS NOT NULL THEN
        ...
        JOIN prefectures p ON m.pref_id = p.pref_id
    ...
    FROM profiles u
    WHERE u.id = l.user_id) as user_info
  FROM listings l
